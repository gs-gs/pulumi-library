  ...
  validateResource: validateResourceOfType(
    ...
    (bucket, _, reportViolation) => {
      if (!bucket.loggings || bucket.loggings.length === 0) {
        reportViolation(
          `Bucket logging is not defined for bucket: ${bucket.bucket}`
        );
      }
    }
  ),
...
export const s3BucketServiceSideEncryptionEnabled: ResourceValidationPolicy = {
  ...
  validateResource: validateResourceOfType(
    ...
    (bucket, _, reportViolation) => {
      if (
        !bucket.serverSideEncryptionConfiguration?.rule
          ?.applyServerSideEncryptionByDefault?.sseAlgorithm
      ) {
        reportViolation(
          `Bucket default encryption is not enabled for bucket: ${bucket.bucket}`
        );
      }
    }
  ),
